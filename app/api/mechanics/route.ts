...
export async function POST(request: Request) {
  ...
  if (!process.env.RESEND_API_KEY || !process.env.EMAIL_FROM) return reply({ error: 'Mechanic email delivery is not configured.' }, 503);
  await connectMongo();
  ...
  try {
    const sent = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: process.env.EMAIL_FROM, to: [email], subject: 'Your Royal Mechanics workshop account', text: `Welcome to Royal Mechanics.\n\nLogin email: ${email}\nTemporary password: ${generatedPassword}\n\nYou will be required to set a new password at first sign-in.` }) });
