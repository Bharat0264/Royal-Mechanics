...
function PhotoPicker({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { const [error, setError] = useState(''); return <label>{label}<input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={async (e) => { const file = e.target.files?.[0]; if (!file) return; try { setError(''); onChange(await compressPhoto(file)); } catch { setError('Could not prepare this photo. Try another one.'); } }} />{value && <small className="admin-success">Photo added ✓</small>}{error && <small className="admin-error">{error}</small>}</label>; }
